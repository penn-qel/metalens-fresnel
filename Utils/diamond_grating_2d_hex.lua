...
S = S4.NewSimulation()
S:SetLattice({Per,0}, {0.5*Per,0.5*Per*math.sqrt(3)}) -- hex lattice
S:SetNumG(G)
...
if (opts == 'Pow') then
	...
	io.stderr:write('m\treal(S2+)\timag(S2+)\treal(S2-)\timag(S2-)\n')
...
if (opts == 'fields') then
	...
	for x=-Per/2,Per/2,0.01 do
		for y=-1.0,11.0,0.02 do
			_,eyr,_,_,eyi,_ = S:GetEField({x,0,y})
			hxr,_,hzr,hxi,_,hzi = S:GetHField({x,0,y})
			io.stderr:write(x .. '\t' .. y .. '\t' .. hxr .. '\t' .. hxi .. '\t' .. eyr .. '\t' .. eyi .. '\t' .. hzr .. '\t' .. hzi .. '\n')
		end
		io.stderr:write('\n')
	end
end
